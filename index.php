...
<head>
	...
	<title>Metode Forecasting</title>
...
<body>
	<nav class="navbar navbar-default">
		<a href="" class="navbar-brand"><i class="fa fa-student"></i> Aplikasi Metode Forecasting</a>
	</nav>
	<div class="container">
		...
		<table class="table table-stripped table-hovered">
			...
			<?php
				...
				$total_xx = 0;
				$total_xy = 0;
				$query = mysqli_query($conn,"select * from pmb");
			...
			<tr id="form-insert" style="display:none">
				<td>
					...
					<!-- form tambah data -->
					<?php 
						$q_thn = mysqli_query($conn,"select max(periode_pmb) from pmb");
	...
	<form method="post" action="?p=prediksi" class="form-inline">
		...
		Tahun Berikutnya <button type="submit" name="prediksi" class="btn btn-info"><i class="fa fa-refresh"></i> Prediksi</button>
	</form>
	...
	<?php
		...
		if($_GET['p']=='prediksi'){
			$x = $x + $list_pilihan;
			$y = $b0 + $b1 * $x;
			echo "<h3>Prediksi untuk $list_pilihan Tahun Berikutnya adalah $y</h3>";
		}
	?>
	</div>
	<br>	
...
<div class="footer navbar-default">
	<div class="container-fluid text-right">
		<a href="https://syahrulgtx76.my.id/">syahrul </a>
	</div>
</div>
